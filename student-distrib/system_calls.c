...
uint8_t create_PCB(uint8_t* argument,uint8_t length){
    int i;
    uint8_t j;
    PCB_t* parent_pcb;
    for(i=0;i<MAX_PCB;i++){
...
int32_t execute(const uint8_t* command){
    ...
    uint8_t argument[32]={'\0'};     // 32 is the maximum length of argument
    ...
    if((prog[0]=='s') && (prog[1]=='t') && (prog[2]=='a') && ( prog[3]=='t') && ( prog[4]=='u') && ( prog[5]=='s')){
        ...
        printf(" Year: %u Month: %u Day: %u Hour: %u Minute: %u \n", year,month,day,hour,minute);
        term->curprogram[0]='s';
        term->curprogram[1]='h';
        term->curprogram[2]='e';
        term->curprogram[3]='l';
